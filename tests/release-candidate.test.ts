...
describe('Release Candidate packaging configuration', () => {
  it('declares an arm64 DMG, stable bundle id, hardened runtime and entitlements', () => {
    ...
    expect(releaseWorkflow).not.toContain('release:community-update')
    expect(releaseWorkflow).not.toContain('debate-studio-mac-arm64.json')
    expect(releaseWorkflow).not.toContain('arm64.update.tar.gz')
    ...
    expect(existsSync(join(root, 'src', 'main', 'community-update-platform.ts'))).toBe(false)
    expect(existsSync(join(root, 'scripts', 'create-community-update.mjs'))).toBe(false)
    expect(updatePlatform).not.toMatch(/\/Applications|appPath|xattr|osascript|quitAndInstall|launchInstaller/)
    expect(updatePlatform).toContain('openDownloadedUpdate')
