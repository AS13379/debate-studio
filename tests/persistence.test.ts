...
describe('SQLite persistence foundation', () => {
  it('creates and initializes a database on first startup', () => {
    ...
    expect(result.value.migrations.currentVersion()).toEqual({ ok: true, value: 7 })
  ...
  it('applies new migration versions once', () => {
    ...
    expect(manager.migrate()).toMatchObject({
      ok: true,
      value: { fromVersion: 0, toVersion: 8, appliedVersions: [1, 2, 3, 4, 5, 6, 7, 8] }
    })
    expect(manager.migrate()).toMatchObject({
      ok: true,
      value: { fromVersion: 8, toVersion: 8, appliedVersions: [] }
    })
