...
export function DebateQualityPage({ onOpenDebate }: { onOpenDebate(debateId: string): void }) {
  ...
  return <section className="page-stack quality-page" aria-labelledby="quality-title">
    ...
    <section className="panel"><div className="section-heading"><div><strong>Prompt 版本表现</strong><span>仅展示已有结构化评分的样本</span></div></div><div className="quality-prompt-performance">{promptPerformance.map((item) => <div key={item.version}><strong>{item.version}</strong><span>平均 {item.average.toFixed(1)} · {item.count} 场</span></div>)}</div></section>
    ...
    <section className="panel"><div className="section-heading"><div><strong>单场质量</strong><span>点击查看完整复盘</span></div></div>{items.length === 0 ? <div className="empty-state compact"><h2>暂无评分</h2><p>完成一场 Mock 辩论后会自动出现在这里。</p></div> : <div className="quality-history-list">{items.map((item) => <button key={item.debateId} onClick={() => onOpenDebate(item.debateId)}><div><strong>{item.title}</strong><span>{winnerLabel(item.winner)} · {new Date(item.createdAt).toLocaleString('zh-CN')}</span></div><b>{item.averageScore.toFixed(1)}</b><small>{item.evidenceCount} 证据 · {item.turnCount} Turn · Prompt v{item.promptVersion} · {item.models.join(' / ') || '模型未知'} · 费用 {formatCost(costs?.byDebate.find((cost) => cost.debateId === item.debateId)?.totalCost)}</small></button>)}</div>}</section>
  </section>
}
