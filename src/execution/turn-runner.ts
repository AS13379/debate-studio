...
export class TurnRunner {
  ...
  async startTurn(engine: DebateEngine, prompt?: string, retryOfTurnId?: string): Promise<TurnRunResult> {
    ...
    try {
      for await (const event of this.adapter.stream(request)) {
        ...
        if (event.type === 'completed') content = event.response.content
        if (event.type === 'error') {
          return {
            turn: {
              id: turnId,
  ...
  retryTurn(engine: DebateEngine, previousTurn: DebateTurn, prompt?: string): Promise<TurnRunResult> {
    if (previousTurn.status !== 'failed' && previousTurn.status !== 'cancelled') {
      throw new Error('Only failed or cancelled turns can be retried.')
    }
    return this.startTurn(engine, prompt, previousTurn.id)
  }
