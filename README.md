...
Debate Studio 可以在设置中检查 GitHub Releases、显示发布说明、下载对应的 Apple Silicon DMG，并校验 GitHub 提供的文件大小与 SHA-256。默认自动检查开启、自动下载关闭。

当前社区构建未经过 Apple Developer ID 签名和公证，因此应用不会自动覆盖 `/Applications` 中的程序。下载完成后请打开 DMG，把 **Debate Studio** 拖入“应用程序”并选择“替换”。此操作只替换应用本体，不会删除 `~/Library/Application Support/debate-studio` 中的数据库、API Key 和辩论记录。
